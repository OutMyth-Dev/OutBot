# Table of Contents

- [Logs](#logs)
- [Self Hosting](#self-hosting)
- [User data](#user-data-collection)
- [Developer Notes](#developer-notes)

---

# Logs

Logs do not exist. OutBot does **NOT LOG ANYTHING**. OutBot does not plan to re introduce logs.

---

# Self Hosting

OutBot is self hosted. OutBot will never be hosted elsewhere. Self hosting allows you more control over **your own data and privacy**. It allows **you to be in complete control**.

---

# User Data Collection

OutBot does not collect any user data. OutBot cannot log it because it has no logs. OutBot uses no privillaged intents and does not plan to use any. OutBot is trying to move a from the /report and /feedback system it currently has. It is currently underdevelopment and should be released in a couple of days.

---

# Open Source

OutBot is 100% open source. Open source allows users to see the source code, distribute it, contribute, and sell it (depending on the license.) OutBot allows all of the above. Before doing any of them please read [OutBot's License](https://github.com/OutMyth-Dev/OutBot?tab=MIT-1-ov-file)  

OutBot

---

# Developer Notes 

If you have any privacy concerns, please open a GitHub issue, create a ticket on OutMyth's discord server, or use /report.
